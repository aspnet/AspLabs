...
{
    /// <summary>
    /// Provides an <see cref="IWebHookReceiver"/> implementation which support WebHooks generated by Stripe. 
    ...
    {
        internal const string SecretKey = "MS_WebHookReceiverSecret_Stripe";
        internal const string EventUriTemplate = "https://api.stripe.com/v1/events/{0}";
        internal const string TestId = "evt_00000000000000";

        private static readonly string[] ReceiverNames = new string[] { "stripe" };
        ...
        public override async Task<HttpResponseMessage> ReceiveAsync(string receiver, HttpRequestContext context, HttpRequestMessage request)
        {
            ...
            if (request.Method == HttpMethod.Post)
            {
                ...
                if (string.Equals(TestId, id, StringComparison.OrdinalIgnoreCase))
                {
                    return request.CreateResponse();
                }

                // Get data directly from Stripe as we don't know where the event comes from.
                data = await GetEventDataAsync(request, id);
        ...
        /// </summary>
        protected virtual async Task<JObject> GetEventDataAsync(HttpRequestMessage request, string id)
        {
            if (request == null)
            {
                throw new ArgumentNullException("request");
            ...
            string address = string.Format(CultureInfo.InvariantCulture, EventUriTemplate, id);
            ...
            using (HttpResponseMessage rsp = await _httpClient.SendAsync(req))
            {
                ...
                {
                    string msg = string.Format(CultureInfo.CurrentCulture, StripeReceiverResources.Receiver_BadId, id);
