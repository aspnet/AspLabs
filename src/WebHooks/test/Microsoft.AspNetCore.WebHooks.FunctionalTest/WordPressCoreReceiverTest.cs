...
namespace Microsoft.AspNetCore.WebHooks.FunctionalTest
{
    public class WordPressCoreReceiverTest : IClassFixture<WebHookTestFixture<Startup>>
    {
        ...
        {
            // Arrange
            var expectedErrorMessage = "The 'wordpress' WebHook receiver does not support the HTTP " +
        ...
        {
            // Arrange
            var expectedErrorMessage = "A 'wordpress' WebHook request must contain a 'code' query parameter.";
            var content = new StringContent(string.Empty);

            // Act
            var response = await _client.PostAsync("/api/webhooks/incoming/wordpress", content);
        ...
        public async Task WebHookAction_WrongCode_IsBadRequest()
        {
            ...
            var response = await _client.PostAsync(
                ...
                "/api/webhooks/incoming/wordpress?code=01234567890123456789012345678902",
                content);
        ...
        {
            // Arrange
            var expectedErrorMessage = "The 'wordpress' WebHook receiver does not support an empty request body.";
            var content = new StringContent(string.Empty);
