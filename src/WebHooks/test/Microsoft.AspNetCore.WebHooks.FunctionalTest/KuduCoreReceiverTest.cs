...
using KuduCoreReceiver;
using Microsoft.Net.Http.Headers;
using Xunit;

namespace Microsoft.AspNetCore.WebHooks.FunctionalTest
{
    public class KuduCoreReceiverTest : IClassFixture<WebHookTestFixture<Startup>>
    {
        private readonly HttpClient _client;
        private readonly WebHookTestFixture<Startup> _fixture;

        public KuduCoreReceiverTest(WebHookTestFixture<Startup> fixture)
        {
        ...
        {
            // Arrange
            var expectedErrorMessage = "The 'kudu' WebHook receiver does not support the HTTP " +
                $"'{method.Method}' method.";
            var request = new HttpRequestMessage(
                method,
                "/api/webhooks/incoming/kudu?code=01234567890123456789012345678901");
        ...
        {
            // Arrange
            var expectedErrorMessage = "A 'kudu' WebHook request must contain a 'code' query parameter.";
            var content = new StringContent(string.Empty);

            // Act
            var response = await _client.PostAsync("/api/webhooks/incoming/kudu", content);
        ...
        public async Task WebHookAction_WrongCode_IsBadRequest()
        {
            ...
            var response = await _client.PostAsync(
                ...
                "/api/webhooks/incoming/kudu?code=01234567890123456789012345678902",
                content);
        ...
        {
            // Arrange
            var expectedErrorMessage = "The 'kudu' WebHook receiver does not support an empty request body.";
            var content = new StringContent(string.Empty);
        ...
        public async Task WebHookAction_WithBody_Succeeds()
        {
            ...
            var path = Path.Combine("Resources", "RequestBodies", "Kudu.json");
