...
public sealed class XmlComment
{
    ...
    private XmlComment(string xml, XmlCommentParserContext context)
    {
        ...
        if (xml.StartsWith("<doc>", StringComparison.InvariantCulture) && xml.EndsWith("</doc>", StringComparison.InvariantCulture))
        {
            ...
            if (innerXmlTrim.StartsWith("<", StringComparison.InvariantCulture) && innerXmlTrim.EndsWith(">", StringComparison.InvariantCulture))
            {
    ...
    private Dictionary<string, string?>? GetListContent(XPathNavigator navigator, string xpath)
    {
        ...
        foreach (XPathNavigator nav in iterator)
        {
            ...
            if (!string.IsNullOrEmpty(name))
            {
                ...
                {
                    // var path = context.Source?.Remote != null ? Path.Combine(EnvironmentContext.BaseDirectory, context.Source.Remote.Path) : context.Source?.Path;
    ...
    private void ResolveCrefLink(XNode node, string nodeSelector, Action<string, string>? addReference)
    {
        ...
        foreach (var item in nodes)
        {
            ...
            if (!success)
            {
                ...
                if (cref.StartsWith("!:", StringComparison.InvariantCulture))
                {
    ...
    private static IEnumerable<LinkInfo> GetMultipleLinkInfo(XPathNavigator navigator, string selector)
    {
        ...
        foreach (XPathNavigator nav in iterator)
        {
            ...
            if (!string.IsNullOrEmpty(refId))
            {
                yield return new LinkInfo(LinkType.CRef, refId, commentId, altText);
            }
            else if (!string.IsNullOrEmpty(commentId))
            {
                ...
                if (match.Success)
                {
                    ...
                    yield return new LinkInfo(LinkType.CRef, id, commentId, altText);
                }
            }
            else if (!string.IsNullOrEmpty(url))
            {
                yield return new LinkInfo(LinkType.CRef, url, null, altText ?? url);
            }
        }
    }
