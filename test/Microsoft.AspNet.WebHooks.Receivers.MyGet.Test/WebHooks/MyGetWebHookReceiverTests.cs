...
namespace Microsoft.AspNet.WebHooks
{
    ...
    {
        private const string TestAction = "BuildQueuedWebHookEventPayloadV1";
        ...
        {
            // Arrange
            IWebHookReceiver rec = new MyGetWebHookReceiver();
            string expected = "myget";

            // Act
            string actual1 = rec.Name;
            string actual2 = MyGetWebHookReceiver.ReceiverName;
        ...
        public async Task ReceiveAsync_Throws_IfPostIsNotUsingHttps()
        {
            ...
            Assert.Equal("The WebHook receiver 'MyGetWebHookReceiverProxy' requires HTTPS in order to be secure. Please register a WebHook URI of type 'https'.", error.Message);
        ...
        public async Task ReceiveAsync_Succeeds_IfPostPing(string id)
        {
            // Arrange
            Initialize(GetConfigValue(id, TestSecret));
            _postRequest.Content = CreateRequestContent("Microsoft.AspNet.WebHooks.Messages.PingMessage.json");
        ...
        public override void Initialize(string config)
        {
            ...
            HttpConfig.InitializeReceiveMyGetWebHooks();

            _postRequest = new HttpRequestMessage(HttpMethod.Post, TestAddress);
            _postRequest.SetRequestContext(RequestContext);
            _postRequest.Content = CreateRequestContent("Microsoft.AspNet.WebHooks.Messages.BuildQueuedMessage.json");
        }

        private HttpContent CreateRequestContent(string payload)
        {
            string content = EmbeddedResource.ReadAsString(payload);
            return new StringContent(content, Encoding.UTF8, MyGetWebHookReceiver.MediaType);
        }
    }
}
