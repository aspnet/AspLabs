...
{
    public class AzureAlertWebHookReceiverTests : WebHookReceiverTestsBase<AzureAlertWebHookReceiver>
    {
        private const string TestContent = "{\"status\":\"Activated\",\"context\":{\"id\":\"12345\",\"name\":\"action1\"},\"properties\":{}}";
        ...
        public async Task ReceiveAsync_Throws_IfPostIsNotUsingHttps()
        {
            ...
            Assert.Equal("The WebHook receiver 'AzureAlertWebHookReceiverProxy' requires HTTPS in order to be secure. Please register a WebHook URI of type 'https'.", error.Message);
        ...
        public async Task ReceiveAsync_ReturnsError_IfPostHasNoAction()
        {
            ...
            Assert.Equal("The HTTP request body did not contain a required 'name' property.", error.Message);
            ReceiverMock.Protected()
                .Verify<Task<HttpResponseMessage>>("ExecuteWebHookAsync", Times.Never(), TestId, RequestContext, _postRequest, ItExpr.IsAny<IEnumerable<string>>(), ItExpr.IsAny<object>());
        }
